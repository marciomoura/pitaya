...
/// Linear ramp generator that interpolates between initial and final values over a specified duration.
///
...
class linear_ramp {
public:
    /// Configuration structure for the linear ramp
    struct config {
    ...
    /// Configures the ramp parameters
    ...
    /// Updates the ramp and returns the current output value
    ...
    /// Checks if the ramp has completed (duration has elapsed)
    ...
    /// Resets the ramp timer to start from the beginning
    void reset() { _timer.reset(); }

    /// Gets the current elapsed time [s]
