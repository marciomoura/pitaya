...
TEST_F(LookupTable1DTest, HandlesExactGridPoints)
{
    ...
    EXPECT_FLOAT_EQ(table.interpolate(10.0f), 100.0f);
    EXPECT_FLOAT_EQ(table.interpolate(20.0f), 200.0f);
    EXPECT_FLOAT_EQ(table.interpolate(30.0f), 150.0f);
    EXPECT_FLOAT_EQ(table.interpolate(40.0f), 250.0f);
}
...
TEST_F(LookupTable1DTest, PerformsLinearInterpolation)
{
    ...
    EXPECT_FLOAT_EQ(table.interpolate(15.0f), 150.0f);
    // Midpoint between 20 and 30 -> (200 + 150) / 2 = 175
    EXPECT_FLOAT_EQ(table.interpolate(25.0f), 175.0f);
    // Midpoint between 30 and 40 -> (150 + 250) / 2 = 200
    EXPECT_FLOAT_EQ(table.interpolate(35.0f), 200.0f);
    // 25% of the way between 10 and 20 -> 100 + 0.25 * (200 - 100) = 125
    EXPECT_FLOAT_EQ(table.interpolate(12.5f), 125.0f);
}
...
TEST_F(LookupTable1DTest, ClampsValuesOutsideBounds)
{
    pitaya::lookup_table_1d<float, NumX> table(x_axis, y_values);
    // Below lower bound
    EXPECT_FLOAT_EQ(table.interpolate(0.0f), 100.0f);
    EXPECT_FLOAT_EQ(table.interpolate(5.0f), 100.0f);
    // Above upper bound
    EXPECT_FLOAT_EQ(table.interpolate(50.0f), 250.0f);
    EXPECT_FLOAT_EQ(table.interpolate(100.0f), 250.0f);
}
...
TEST_F(LookupTable1DTest, ConfigureMethodWorks)
{
    ...
    EXPECT_FLOAT_EQ(table.interpolate(0.5f), -5.0f);
    EXPECT_FLOAT_EQ(table.interpolate(2.5f), -25.0f);
    EXPECT_FLOAT_EQ(table.interpolate(-1.0f), 0.0f);  // Clamping with new data
}
